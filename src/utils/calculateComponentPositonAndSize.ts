...
function calculateLeftTop({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
...
function calculateRightTop({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint } = pointInfo
  const { rotate = 0 } = style
  let newCenterPoint = getCenterPoint(curPositon, symmetricPoint)
...
function calculateRightBottom({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint } = pointInfo
  const { rotate = 0 } = style
  let newCenterPoint = getCenterPoint(curPositon, symmetricPoint)
...
function calculateLeftBottom({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint } = pointInfo
  const { rotate = 0 } = style
  let newCenterPoint = getCenterPoint(curPositon, symmetricPoint)
...
function calculateTop({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  ...
  style.left = Math.round(newCenter.x - ((style?.width || 0) / 2))
}

function calculateRight({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint, curPoint } = pointInfo
  const { rotate = 0 } = style
  const rotatedcurPositon = calculateRotatedPointCoordinate(curPositon, curPoint, -rotate)
  ...
  style.height = height
  style.width = Math.round(newWidth)
  style.top = Math.round(newCenter.y - ((style?.height || 0) / 2))
  style.left = Math.round(newCenter.x - (newWidth / 2))
}

function calculateBottom({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint, curPoint } = pointInfo
  const { rotate = 0 } = style
  const rotatedcurPositon = calculateRotatedPointCoordinate(curPositon, curPoint, -rotate)
...
function calculateLeft({ style, curPositon, proportion, needLockProportion, pointInfo }: Props) {
  const { symmetricPoint, curPoint } = pointInfo
  const { rotate = 0 } = style
  const rotatedcurPositon = calculateRotatedPointCoordinate(curPositon, curPoint, -rotate)
